...
import 'package:helnay/screen/get_started_screen/widget/eula_sheet.dart';
...
class GetStartedScreenViewModel extends BaseViewModel {
      ...
      ApiProvider().getProfile(userID: ConstRes.aUserId).then(
        (value) {
          if (value?.data?.age == null) {
            ...
            Get.off(() => const DashboardScreen(), transition: Transition.zoom);
          }
        },
      );
    } else {
  ...
  void onSkipTap() {
    Get.off(() => const LoginDashboardScreen(), transition: Transition.zoom);
  }
