...
import 'package:helnay/screen/map_screen/widgets/user_pop_up.dart';
        ...
        return Marker(
          ...
          onTap: () {
            for (var user in cluster.items) {
              onUserProfileTap(
                  target: LatLng(double.parse(user.userData?.lattitude ?? ''),
                      double.parse(user.userData?.longitude ?? '')),
                  data: user.userData,
